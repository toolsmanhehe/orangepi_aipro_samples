...
std::string Brack(const std::string &v)
{
    return R"(")" + v + R"(")";
}
...
template <typename T> bool IsEnd(const T &v)
{
    return false;
}

template <> bool IsEnd(const char &v)
{
    return v == kEnd;
}
...
struct OutputDef {
    ...
    void GenCode(Code &ss) const
    {
        if (is_dynamic) {
            ss << sig << " = []" << kEnd;
            ss << "for i in range(output_index, output_index + size_of_" << sig << ")" << ":" << kEnd;
...
template <> struct Value<Type> {
    static std::string ToString(const Type &v)
    {
        ...
        if (v == Type::DT_RESOURCE)
            return "DataType.DT_RESOURCE";
        if (v == Type::DT_VARIANT)
...
template <typename T> struct Value<std::vector<std::vector<T>>> {
